...
export default function LoginPage() {
  ...
  async function handleEmail(e) {
    ...
    try {
      ...
      await checkAndRedirect(cred.user.uid);
    } catch (err) {
    ...
    <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-void)', padding: '24px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '32px', gap: '8px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '15px' }}>Welcome back! Sign in to continue.</p>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '24px', padding: '40px', boxShadow: '0 0 60px rgba(124,58,237,0.15)' }}>
          ...
          <form onSubmit={handleEmail} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <input id="login-email" type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} required className="glow-input" style={{ padding: '14px 16px', borderRadius: '12px', fontSize: '15px', width: '100%' }} />
            <input id="login-password" type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required className="glow-input" style={{ padding: '14px 16px', borderRadius: '12px', fontSize: '15px', width: '100%' }} />
            ...
            <button type="submit" id="login-submit" disabled={loading} className="btn-primary" style={{ padding: '14px', borderRadius: '12px', fontSize: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              {loading ? (
                <><div style={{ width: '16px', height: '16px', border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid white', borderRadius: '50%', animation: 'rotate-border 0.8s linear infinite' }} />Signing in...</>
              ) : 'Sign In →'}
            </button>
          </form>
          ...
          <p style={{ textAlign: 'center', marginTop: '24px', color: 'var(--text-secondary)', fontSize: '14px' }}>
            ...
            <Link href="/signup" style={{ color: 'var(--accent-violet)', textDecoration: 'none', fontWeight: 600 }}>Sign up free</Link>
          </p>
        </div>
      </div>
    </main>
